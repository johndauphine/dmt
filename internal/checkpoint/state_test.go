...
// TestGetAITuningAggregatesByWaw is an integration test for the SQL-side
// aggregator added in issue #141. The smartconfig prompt's per-waw retry-rate
// summary is built from these aggregates, so the SQL must group by
// write_ahead_writers and exclude rows without a completed run (the
// final_throughput=0 filter), matching the prior Go-side aggregation.
func TestGetAITuningAggregatesByWaw(t *testing.T) {
	state, err := New(t.TempDir())
	if err != nil {
		t.Fatalf("New() error: %v", err)
	}
	...
	// 4 waw=1 runs (3 clean, 1 retried), 2 waw=2 runs (both clean), 1 incomplete
	// run that should be excluded by the final_throughput > 0 filter.
	type fixture struct {
		waw          int
		throughput   float64 // 0 means "don't update", leaves NULL
		durationSecs float64
		retries      int
	}
	fx := []fixture{
		{waw: 1, throughput: 900000, durationSecs: 22, retries: 0},
		{waw: 1, throughput: 950000, durationSecs: 21, retries: 0},
		{waw: 1, throughput: 1000000, durationSecs: 20, retries: 0},
		{waw: 1, throughput: 500000, durationSecs: 40, retries: 3},
		{waw: 2, throughput: 1100000, durationSecs: 18, retries: 0},
		{waw: 2, throughput: 1050000, durationSecs: 19, retries: 0},
	...
	aggs, err := state.GetAITuningAggregatesByWaw("mssql", "postgres")
	if err != nil {
		t.Fatalf("GetAITuningAggregatesByWaw: %v", err)
	}
	// Expect waw=1 and waw=2 only (waw=4 row was incomplete, excluded).
	if len(aggs) != 2 {
		t.Fatalf("expected 2 waw groups, got %d: %+v", len(aggs), aggs)
	}
	if aggs[0].WriteAheadWriters != 1 {
		t.Errorf("aggs[0].WriteAheadWriters = %d, want 1 (sorted ASC)", aggs[0].WriteAheadWriters)
	}
	if aggs[0].TotalRuns != 4 {
		t.Errorf("waw=1 TotalRuns = %d, want 4", aggs[0].TotalRuns)
	}
	if aggs[0].RunsWithRetries != 1 {
		t.Errorf("waw=1 RunsWithRetries = %d, want 1", aggs[0].RunsWithRetries)
	}
	if aggs[0].TotalRetries != 3 {
		t.Errorf("waw=1 TotalRetries = %d, want 3", aggs[0].TotalRetries)
	}
	if math.Abs(aggs[0].PeakThroughput-1000000) > 1 {
		t.Errorf("waw=1 PeakThroughput = %f, want 1000000", aggs[0].PeakThroughput)
	}
	if aggs[1].WriteAheadWriters != 2 {
		t.Errorf("aggs[1].WriteAheadWriters = %d, want 2", aggs[1].WriteAheadWriters)
	}
	if aggs[1].TotalRuns != 2 {
		t.Errorf("waw=2 TotalRuns = %d, want 2", aggs[1].TotalRuns)
	}
	if aggs[1].RunsWithRetries != 0 {
		t.Errorf("waw=2 RunsWithRetries = %d, want 0", aggs[1].RunsWithRetries)
	}
}

// TestGetAITuningAggregatesByChunkSize mirrors TestGetAITuningAggregatesByWaw
// but for chunk_size groups. The aggregate must correctly average across runs.
func TestGetAITuningAggregatesByChunkSize(t *testing.T) {
	state, err := New(t.TempDir())
	if err != nil {
		t.Fatalf("New() error: %v", err)
	}
	...
	// 2 runs at chunk_size=50000 (avg 950k), 1 run at chunk_size=100000 (1.1M).
	specs := []struct {
		chunk      int
		throughput float64
	}{
	...
	aggs, err := state.GetAITuningAggregatesByChunkSize("mssql", "postgres")
	if err != nil {
		t.Fatalf("GetAITuningAggregatesByChunkSize: %v", err)
	}
	if len(aggs) != 2 {
		t.Fatalf("expected 2 chunk_size groups, got %d: %+v", len(aggs), aggs)
	}
	if aggs[0].ChunkSize != 50000 || aggs[0].Runs != 2 {
		t.Errorf("aggs[0] = %+v, want chunk_size=50000 runs=2", aggs[0])
	}
	if math.Abs(aggs[0].AvgThroughput-950000) > 1 {
		t.Errorf("chunk_size=50000 AvgThroughput = %f, want 950000", aggs[0].AvgThroughput)
	}
	if aggs[1].ChunkSize != 100000 || aggs[1].Runs != 1 {
		t.Errorf("aggs[1] = %+v, want chunk_size=100000 runs=1", aggs[1])
	}
}
