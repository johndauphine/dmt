...
func (s *SmartConfigAnalyzer) formatHistoricalContext() string {
	...
	if err == nil && len(tuningHistory) > 0 {
		...
		for i := len(tuningHistory) - 1; i >= 0; i-- {
			...
			sb.WriteString(fmt.Sprintf("     workers=%d, chunk_size=%d, read_ahead=%d, write_ahead=%d, parallel_readers=%d, max_partitions=%d, large_table_threshold=%d, source_conns=%d, target_conns=%d",
