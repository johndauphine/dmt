...
2. IDENTITY/AUTO-INCREMENT COLUMNS - use explicit sequences (NOT identity columns):
   - Create the column as: COLUMN_NAME NUMBER NOT NULL
   - Create a separate sequence: CREATE SEQUENCE SCHEMA.SEQ_TABLENAME_COLNAME START WITH 1 INCREMENT BY 1 NOCACHE
   - NEVER use GENERATED AS IDENTITY (system-generated sequences cannot be reset after migration)
   - The sequence name format MUST be: SEQ_<TABLENAME>_<COLUMNNAME> (all uppercase)
   - Include the CREATE SEQUENCE statement BEFORE the CREATE TABLE statement
