...
func TestEnforceNvarchar(t *testing.T) {
	tests := []struct {
		name     string
		input    string
		expected string
		...
		{
			name:     "varchar to nvarchar",
			input:    "CREATE TABLE t (name VARCHAR(40), id INT);",
			expected: "CREATE TABLE t (name NVARCHAR(40), id INT);",
		},
		{
			name:     "char to nchar",
			input:    "CREATE TABLE t (code CHAR(10));",
			expected: "CREATE TABLE t (code NCHAR(10));",
		},
		{
			name:     "varchar max",
			input:    "CREATE TABLE t (body VARCHAR(MAX));",
			expected: "CREATE TABLE t (body NVARCHAR(MAX));",
		},
		{
			name:     "nvarchar preserved",
			input:    "CREATE TABLE t (name NVARCHAR(40));",
			expected: "CREATE TABLE t (name NVARCHAR(40));",
		},
		{
			name:     "mixed case",
			input:    "CREATE TABLE t (a varchar(100), b Varchar(50), c VARCHAR(MAX));",
			expected: "CREATE TABLE t (a NVARCHAR(100), b NVARCHAR(50), c NVARCHAR(MAX));",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := enforceNvarchar(tt.input)
			if got != tt.expected {
				t.Errorf("enforceNvarchar():\n  got:  %s\n  want: %s", got, tt.expected)
			}
		})
	}
}
