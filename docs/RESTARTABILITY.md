...
- **Idempotent INSERT on resume.** When the writer pool detects a resume run (`initialRowNum > startRow`) for a ROW_NUMBER-paged table in non-upsert mode, it routes writes through driver-specific idempotent paths so replayed already-committed rows are silent no-ops:
