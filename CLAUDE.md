...
2. **Smart Config** (`driver/ai_smartconfig.go`) — Queries source stats + system resources (gopsutil), recommends config. `config.ApplyAISuggestions()` only overrides params the user didn't explicitly set (tracked via `AutoConfig.Original*` fields). The smartconfig prompt encodes a "RULE 1" retry-rate override that downgrades `write_ahead_writers` when the historical retry rate at the candidate value exceeds zero. **Compliance with this specific override** (not overall tuning quality) **was tested across 7 model IDs / 4 families**: only `claude-haiku-4-5-20251001` followed it consistently. The other tested models — `google/gemma-4-e4b`, `google/gemma-4-26b-a4b`, `google/gemma-4-31b`, `qwen/qwen3.6-27b`, `openai/gpt-oss-20b`, and OpenAI cloud `gpt-5.5` — anchored on the stated baseline `write_ahead_writers=2` regardless of how the rule was structured. (Anthropic Sonnet not tested; the README's "Haiku and Sonnet produce identical tuning results" claim is about overall tuning quality and is unrelated to this specific override.) **For parameter tuning, recommended model is `claude-haiku-4-5-20251001`**; users on other providers/models can expect occasional `write_ahead_writers=2` choices that produce 10-40% chunk-retry rates on virtualized network transports (Docker Desktop on macOS/Windows, WSL2). See PRs #133 and #132 for the investigation.
