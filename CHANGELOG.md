...
- SQLite incremental sync now accepts explicitly configured `TEXT` date
  ...
  sync (#311).
- Upsert validation now treats the target as a permitted row-count superset,
  so source-side deletes retained on the target do not fail daily incremental
  runs while missing target rows still fail (#310).
